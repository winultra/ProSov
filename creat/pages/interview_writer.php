...
<div class="wrapper_edit_history">
    ...
    <div style="text-align:right;"><button class="buttonStyle" id="send_otchet_cdo" onclick="add_video_interviewwriter()">Добавить</button></div>
</div>
<?
}else{
...
?>
    ...
    <?while($queryAuthrar = mysqli_fetch_assoc($queryAuthr)){?>
            <?if($queryAuthrar['services'] == 'youtube'){?>
                <div class="author_videos">
                    <i class="mdi mdi-close" title="Удалить видео" onclick="delete_video_interviewwriter(<?=$queryAuthrar['id']?>)"></i>
                    <a href="/literary-life/Interview-writer?id=<?=$queryAuthrar['id']?>" target="_tblank">
