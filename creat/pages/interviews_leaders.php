...
<div class="wrapper_edit_history">
    ...
    <div style="text-align:right;"><button class="buttonStyle" id="send_otchet_cdo" onclick="add_video_interviews_leaders()">Добавить</button></div>
</div>
<?
}else{
...
?>
    ...
    <a href="/creat/interviews-leaders-add">
    ...
    <?while($queryAuthrar = mysqli_fetch_assoc($queryAuthr)){?>
            <?if($queryAuthrar['services'] == 'youtube'){?>
                <div class="author_videos">
                    <i class="mdi mdi-close" title="Удалить видео" onclick="delete_video_interviews_leaders(<?=$queryAuthrar['id']?>)"></i>
                    <a href="/about-modernity/interviews-leaders?id=<?=$queryAuthrar['id']?>" target="_tblank">
